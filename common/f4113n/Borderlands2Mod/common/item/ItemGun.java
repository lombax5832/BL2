...
//    	par3List.add(gunNames[par1ItemStack.getItemDamage()]);
    ...
    public void calcDmg(GunAtributes atr, boolean hasAmp, int Amp, List list){
    	...
    	String damageString = new  StringBuilder().append(damageColor).append(Damage).toString();
    	
    	list.add(damageString);
    	
    	if(hasAmp || atr.pellets > 1){
    		String damagePerShotString = new  StringBuilder().append(damagePerShotColor).append(dmgShot).append(dmg).toString();
